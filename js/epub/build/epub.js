...
*/
  ...
  var rsvp =
    ...
    _defineProperty(_async$filter, "async", async),
    _defineProperty(
      _async$filter,
      "filter", // babel seems to error if async isn't a computed prop here...
      filter$1
    ),
    _async$filter);
...
(function(root) {
  ...
  if (typeof define === "function" && define.amd) {
    //AMD
    define(["rsvp", "jszip", "localforage"], function(
      RSVP,
      JSZip,
      localForage
    ) {
...
EPUBJS.Book = function(options) {
  ...
  if (
    typeof this.settings.bookPath === "string" ||
...
EPUBJS.Book.prototype.loadPackage = function(_containerPath) {
  ...
  if (!this.settings.packageUrl) {
    //-- provide the packageUrl to skip this step
    packageXml = book
      .loadXml(book.bookUrl + containerPath)
...
EPUBJS.Book.prototype.unpack = function(packageXml) {
  ...
  if (book.contents.coverPath) {
    book.cover = book.contents.cover =
      book.settings.contentsPath + book.contents.coverPath;
  }
  ...
  book.locations = new EPUBJS.Locations(
    book.spine,
    book.store,
    book.settings.withCredentials
  );
...
EPUBJS.Book.prototype.generatePageList = function(width, height, flag) {
  ...
  var nextChapter = function(deferred) {
    ...
    if (next >= spineLength) {
      ...
      pager
        .displayChapter(chapter, this.globalLayoutProperties)
        .then(function(chap) {
...
EPUBJS.Book.prototype.generatePagination = function(width, height, flag) {
  ...
  this.ready.spine.promise.then(function() {
    book.generatePageList(width, height, flag).then(
      function(pageList) {
        book.pageList = book.contents.pageList = pageList;
...
EPUBJS.Book.prototype.listenToRenderer = function(renderer) {
  ...
  renderer.on(
    ...
    function(range) {
      ...
      //this routine is called whenever you resize screen
      //if (debugMode == "TRUE") {
      //  alert("renderer:visibleRangeChanged -- line 3275");
     // }
      ...
      if (this.pageList.length > 0) {
        ...
        if (debugMode == "TRUE") {
          alert("trigger(book:pageChanged -- line 3290");
        }
...
EPUBJS.Book.prototype.loadChange = function(url) {
  var debugMode = localStorage.getItem("debugMode", "FALSE");
      if (debugMode == "TRUE") {
        alert("EPUBJS.Book.prototype.loadChange -- line 3309");
      }
...
EPUBJS.Book.prototype.coverUrl = function() {
  var retrieved = this.ready.cover.promise.then(
    function(url) {
...
EPUBJS.Book.prototype.generateBookKey = function(identifier) {
  return (
    "epubjs:" + EPUBJS.VERSION + ":" + window.location.host + ":" + identifier
  );
};
...
EPUBJS.Book.prototype.renderTo = function(elem) {
  ...
  rendered = this.opened.then(function() {
    ...
    book.renderer.initialize(
      book.element,
      book.settings.width,
      book.settings.height
    );
...
EPUBJS.Book.prototype.displayChapter = function(chap, end, deferred) {
  console.log("I am displaying a chapter --  line 3398 ");
  console.log(chap + " is chapter and end is " + end + " line 3399");
  ...
  if (!this.isRendered) {
    ...
    alert("Bob this is where the error is on line 3414");
    defer.reject({
      message: "Rendering",
  ...
  if (EPUBJS.core.isNumber(chap)) {
    console.log("chapter is number -- line 3435");
    pos = chap;
  } else {
    cfi = new EPUBJS.EpubCFI(chap);
    console.log("chapter is not a number. here is cfi -- line 3439");
    console.log(cfi);
    pos = cfi.spinePos;
  }
  console.log(
    pos + " Is Pos and spine length is " + this.spine.length + "-- line 3443"
  );
  if (pos < 0 || pos >= this.spine.length) {
    console.warn("Not A Valid Location -- line 3441");
    pos = 0;
    end = false;
    cfi = false;
  }
  ...
  render.then(
    function(rendered) {
      ...
      if (
        book.settings.fromStorage === false &&
...
EPUBJS.Book.prototype.nextPage = function(defer) {
  ...
  // if (debugMode == "TRUE") {
  //we arrive here
  // alert(" in EPUBJS.Book.prototype.nextPage -- line 3687");
  //}
  // here is where we change the content.
  ...
  if (debugMode == "TRUE") {
    // we got here
    alert(" leaving EPUBJS.Book.prototype.nextPage n-- line 3696");
  }
  return defer.promise;
};
...
EPUBJS.Book.prototype.nextChapter = function(defer) {
  ...
  if (this.spinePos < this.spine.length - 1) {
    ...
    while (
      this.spine[next] &&
      this.spine[next].linear &&
      this.spine[next].linear == "no"
    ) {
      next++;
    }
...
EPUBJS.Book.prototype.prevChapter = function(defer) {
  ...
  if (this.spinePos > 0) {
    ...
    while (
      this.spine[prev] &&
      this.spine[prev].linear &&
      this.spine[prev].linear == "no"
    ) {
      prev--;
    }
    if (prev >= 0) {
...
EPUBJS.Book.prototype.goto = function(target) {
  ...
  if (target.indexOf("epubcfi(") === 0) {
    ...
    return this.gotoPercentage(
      parseInt(target.substring(0, target.length - 1)) / 100
    );
...
EPUBJS.Book.prototype.gotoHref = function(url, defer) {
  ...
  split = url.split("#");
  console.log("split --  line 3673");
  chapter = split[0];
  section = split[1] || false;
  if (chapter.search("://") == -1) {
    relativeURL = chapter.replace(
      EPUBJS.core.uri(this.settings.contentsPath).path,
      ""
    );
  } else {
...
EPUBJS.Book.prototype._registerReplacements = function(renderer) {
  renderer.registerHook(
    "beforeChapterDisplay",
    this.applyStyles.bind(this, renderer),
    true
  );
  renderer.registerHook(
    "beforeChapterDisplay",
    this.applyHeadTags.bind(this, renderer),
    true
  );
  renderer.registerHook(
    "beforeChapterDisplay",
    this.applyClasses.bind(this, renderer),
    true
  );
  renderer.registerHook(
    "beforeChapterDisplay",
...
EPUBJS.Book.prototype.parseLayoutProperties = function(metadata) {
  ...
  var spread =
    ...
    "auto";
  var orientation =
    (this.settings.layoutOveride && this.settings.layoutOveride.orientation) ||
...
EPUBJS.Chapter.prototype.cfiFromRange = function(_range) {
  ...
  if (typeof document.evaluate != "undefined") {
    ...
    startContainer = this.document.evaluate(
      startXpath,
      this.document,
    ...
    if (!_range.collapsed) {
      endContainer = this.document.evaluate(
        endXpath,
        this.document,
        EPUBJS.core.nsResolver,
        XPathResult.FIRST_ORDERED_NODE_TYPE,
    ...
    // Fuzzy Match
    if (!startContainer) {
      console.log("not found, try fuzzy match -- line 4235");
      cleanStartTextContent = EPUBJS.core.cleanStringForXpath(
        _range.startContainer.textContent
      );
      ...
      if (startContainer) {
        ...
        if (!_range.collapsed) {
          cleanEndTextContent = EPUBJS.core.cleanStringForXpath(
...
EPUBJS.Chapter.prototype.replaceWithStored = function(
  query,
  attr,
  func,
  callback
...
EPUBJS.EpubCFI = function(cfiStr) {
  console.log("I entered EPUBJS.EpubCFI -- line 5393");
...
EPUBJS.Layout.Reflowable.prototype.format = function(
  documentElement,
  _width,
  _height,
  _gap
...
EPUBJS.Layout.ReflowableSpreads.prototype.format = function(
  documentElement,
  _width,
  _height,
  _gap
...
EPUBJS.Layout.ReflowableSpreads.prototype.calculatePages = function() {
  ...
  this.documentElement.style.width =
    displayedPages * this.spreadWidth - this.gap + "px";
...
EPUBJS.Layout.Fixed.prototype.format = function(
  documentElement,
  _width,
  _height,
  _gap
...
EPUBJS.Pagination.prototype.pageFromCfi = function(cfi) {
  ...
  var index = EPUBJS.core.indexOfSorted(
    cfi,
    this.locations,
...
EPUBJS.Parser.prototype.spine = function(spineXml, manifest) {
  ...
  return items.map(function(item, index) {
    ...
    var manifestPropArray = manifestProps.length
      ? manifestProps.split(" ")
      : [];
    return {
      id: Id,
...
EPUBJS.Parser.prototype.navItems = function(
  navNode,
  spineIndexByURL,
  bookSpine
) {
...
EPUBJS.Parser.prototype.pageList = function(
  navHtml,
  spineIndexByURL,
  bookSpine
) {
...
EPUBJS.Parser.prototype.pageListItem = function(
  item,
  spineIndexByURL,
  bookSpine
) {
...
EPUBJS.Render.Iframe = function() {
  // var debugMode = localStorage.getItem("debugMode", "FALSE");
  // if (debugMode == "TRUE") {
  //   alert('I am in EPUBJS.Render.Iframe -- line 7201');
  // I go here first
  //}
  this.iframe = null;
  this.document = null;
...
EPUBJS.Render.Iframe.prototype.create = function() {
  ...
  // if (debugMode == "TRUE") {
  // I come here second
  //   alert('I am in EPUBJS.Render.Iframe.prototype.create -- line 7215');
  // }
...
EPUBJS.Render.Iframe.prototype.addIframe = function() {
  ...
  //if (debugMode == "TRUE") {
  // yes we go here.
  //  alert('I am at start of  EPUBJS.Render.Iframe.prototype.addIframe -- line 7233');
  // }
  ...
  //if (debugMode == "TRUE") {
  // alert('I am at end of  EPUBJS.Render.Iframe.prototype.addIframe -- line 7253');
  // alert(JSON.stringify(this.iframe.id, null, 4));
  //}
  return this.iframe;
};
...
EPUBJS.Render.Iframe.prototype.load = function(contents, url) {
  ...
  // if (debugMode == "TRUE") {
  // I come here third
  //   alert('I am in EPUBJS.Render.Iframe.prototype.load-- line 7258');
  // }
  var render = this,
  ...
  this.iframe.onload = function(e) {
    ...
    render.bodyEl =
      render.document.body || render.document.querySelector("body");
    render.window = render.iframe.contentWindow;

    render.window.addEventListener(
      "resize",
      render.resized.bind(render),
      false
    );
...
EPUBJS.Render.Iframe.prototype.setPageDimensions = function(
  pageWidth,
  pageHeight
) {
...
//-- Show the page containing an Element
EPUBJS.Render.Iframe.prototype.getPageNumberByRect = function(
  boundingClientRect
) {
  var left, pg;
...
EPUBJS.Renderer.prototype.load = function(contents, url) {
  ...
  this.render.load(contents, url).then(
    function(contents) {
...
EPUBJS.Renderer.prototype.reformat = function() {
  ...
  renderer.formated = renderer.layout.format(
    renderer.render.docEl,
    renderer.render.width,
    renderer.render.height,
    renderer.gap
  );
  renderer.render.setPageDimensions(
    renderer.formated.pageWidth,
    renderer.formated.pageHeight
  );
...
EPUBJS.Renderer.prototype.page = function(pg) {
  var debugMode = localStorage.getItem("debugMode", "FALSE");
  //if (debugMode == "TRUE") {
  //  alert("Starting EPUBJS.Renderer.prototype.page -- line 8004");
  //}
  if (!this.pageMap) {
    if (debugMode == "TRUE") {
      alert("pageMap not set, queuing");
    }
  ...
  if (debugMode == "TRUER") {
    alert(
      pg +
        " is page and this.diplayedPages is " +
        this.displayedPages +
        " -- line 8016"
    );
  }
  if (pg >= 1 && pg <= this.displayedPages) {
    this.chapterPos = pg;
    if (debugMode == "TRUER") {
      alert("chapterPos " + pg + " passed if statement -- line 8021");
    }
    this.render.page(pg);
    if (debugMode == "TRUER") {
      alert("passed render.page(pg) statement -- line 8025");
    }
    this.visibleRangeCfi = this.getVisibleRangeCfi();
    this.currentLocationCfi = this.visibleRangeCfi.start;
    if (debugMode == "TRUER") {
      alert("currentLocationCfi " + this.currentLocationCfi + "-- line 8031");
      alert(JSON.stringify(this.visibleRangeCfi));
    }
    this.trigger("renderer:locationChanged", this.currentLocationCfi);
    this.trigger("renderer:visibleRangeChanged", this.visibleRangeCfi);
  ...
  if (debugMode == "TRUE") {
    alert("Return false beause page is greater than the total");
  }
  return false;
};
...
EPUBJS.Renderer.prototype.nextPage = function() {
  //var debugMode = localStorage.getItem("debugMode", "FALSE");
  //if (debugMode == "TRUE") {
  //	var newPos = this.chapterPos + 1;
  //	alert("EPUBJS.Renderer.prototype.nextPage will now show page " + newPos);
  //	alert(this.page(this.chapterPos + 1));
  //}
...
EPUBJS.Renderer.prototype.firstElementisTextNode = function(node) {
  ...
  if (
    ...
    children[0].textContent.trim().length
  ) {
    // With non whitespace or return characters
    return true;
  }
  return false;
};
...
EPUBJS.Renderer.prototype.removeEventListeners = function() {
  ...
  this.listenedEvents.forEach(function(eventName) {
    this.render.document.removeEventListener(
      eventName,
      this.triggerEvent,
      false
    );
...
EPUBJS.replace.head = function(callback, renderer) {
  renderer.replaceWithStored(
    "link[href]",
    "href",
    EPUBJS.replace.links,
    callback
  );
};
...
EPUBJS.replace.links = function(_store, full, done, link) {
  ...
  if (link.getAttribute("rel") === "stylesheet") {
    EPUBJS.replace.stylesheets(_store, full).then(
      function(url, full) {
        // done
        done(url, full);
      },
...
EPUBJS.replace.stylesheets = function(_store, full) {
  ...
  _store.getText(full).then(
    function(text) {
      var url;

      EPUBJS.replace.cssImports(_store, full, text).then(
        function(importText) {
          text = importText + text;

          EPUBJS.replace.cssUrls(_store, full, text).then(
            function(newText) {
...
EPUBJS.replace.cssUrls = function(_store, base, text) {
  ...
  var promises = matches.map(function(str) {
    var full = EPUBJS.core.resolveUrl(
      base,
      str.replace(/url\(|[|\)|\'|\"]|\?.*$/g, "")
    );
    return _store.getUrl(full).then(
      function(url) {
...
EPUBJS.Storage.prototype.getText = function(url) {
  ...
  return EPUBJS.core
    .request(url, "arraybuffer", this.withCredentials)
    .then(
      function(buffer) {
        if (this.offline) {
          this.offline = false;
...
EPUBJS.Unarchiver.prototype.open = function(zipUrl, callback) {
  if (zipUrl instanceof ArrayBuffer) {
    ...
    return EPUBJS.core.request(zipUrl, "binary").then(
      function(data) {
...
EPUBJS.Unarchiver.prototype.toStorage = function(entries) {
  ...
  console.log("time " + timeout + " -- line 9048");
